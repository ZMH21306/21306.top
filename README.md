# 🚀 21306.top

> 个人项目展示与资源收集网站

[![项目状态](https://img.shields.io/badge/项目状态-活跃中-success?style=flat-square&logo=github)](https://github.com/Zmh20121211/21306.top)
[![许可证](https://img.shields.io/badge/许可证-GPL--3.0-blue?style=flat-square&logo=gnu)](LICENSE)
[![最后更新](https://img.shields.io/badge/最后更新-2025年6月13日-brightgreen?style=flat-square&logo=calendar)](README.md)

[English](README_EN.md) | [中文](README.md)

---

## 📖 项目简介

**21306.top** 是一个个人项目展示与资源收集网站，提供登录界面、视频播放、模拟时钟、资源收集等功能模块。项目采用纯前端技术栈构建，无需后端服务，可直接部署到任意静态托管平台（如 GitHub Pages）。

## ✨ 功能特性

| 模块 | 说明 |
|------|------|
| 🔐 **登录界面** | 用户登录入口，支持图标与交互效果 |
| ▶️ **视频播放器** | 支持 URL 自定义播放、自动播放、音量调节、进度条、多端兼容 |
| ⏱️ **模拟时钟** | 模拟时钟界面，支持图标与全屏按钮 |
| 📊 **资源收集页** | 资源分类展示、页面图标、鼠标交互效果 |
| 🚧 **404 页面** | 自定义 404 错误页面 |

## 🗂️ 项目结构

```
21306.top/
├── index.html          # 主页面（项目展示）
├── 404.html            # 404 错误页面
├── clock/              # 模拟时钟模块
│   └── index.html
├── vsbm/               # 子模块
│   └── index.html
├── wrc_rally.html      # 独立页面
├── 测试.html           # 测试页面
├── arduino/            # Arduino 相关资源
├── favicon.ico         # 站点图标
├── CNAME               # 自定义域名配置
├── README.md           # 项目说明（中文）
└── README_EN.md        # 项目说明（英文）
```

## 🛠️ 技术栈

- **HTML5** — 页面结构
- **CSS3** — 样式与主题（支持亮/暗主题切换）
- **JavaScript** — 交互逻辑
- **GitHub Pages** — 静态托管

## 🚧 当前开发：新版视频播放器

| 完成日期 | 功能 | 详细说明 |
|----------|------|----------|
| 2025年5月25日 | ✨ 界面美化 | 实现更美观的用户界面设计 |
| 2025年6月6日 | ▶️ 自动播放 | 实现打开页面自动播放功能 |
| 2025年6月7日 | 📱 多端兼容 | 修正手机与电脑端兼容问题 |
| 2025年6月8日 | 📏 进度条优化 | 增长进度条占比，提升用户体验 |
| 2025年6月11日 | 🔗 URL定义框 | 增加视频URL定义功能 |
| 2025年6月11日 | 🎚️ 音量调节 | 优化音量调节精度问题 |
| 2025年6月11日 | ⏱️ 播放时间 | 修复已播放时间波动问题 |
| 2025年6月12日 | 🔊 音量预设 | 页面启动时音量设为50% |
| 2025年6月13日 | 🔧 持续优化 | 修复问题与性能优化（开发中） |

## 📅 开发时间线

### 2025年第一季度

| 日期 | 开发里程碑 |
|------|-----------|
| 2025年1月25日 | ✅ **项目启动** - 建站 |
| 2025年2月21日 | ✅ **登录界面** - 核心功能完工 |
| 2025年3月17日 | ✅ **视频播放** - 界面完工 |
| 2025年3月18日 | 🔄 视频播放界面推进更新 |
| 2025年3月12日 | 📝 在自述文件中添加 "Star 历史" |
| 2025年3月13日 | 🔧 自述文件推进更新 |

### 2025年第二季度

| 日期 | 开发里程碑 |
|------|-----------|
| 2025年4月22日 | 🔄 登录后界面推进更新 |
| 2025年4月25-26日 | 🔄 结构重构 - 调整页面目录位置及推进更新 |
| 2025年4月26-27日 | ➕ 添加404界面及推进更新 |
| 2025年4月28日 | ⏱️ 添加模拟时钟界面 |
| 2025年4月28日 | 🖼️ 登录界面图标更新 |
| 2025年4月28日 | 🐞 修复图标更改导致的BUG |
| 2025年4月28日 | 📁 模拟时钟界面文件重命名 |
| 2025年4月28日 | 🐞 修复文件名导致的小BUG |
| 2025年4月28日 | 🐞 修复路径错误BUG |
| 2025年4月28日 | 🔄 登录后界面大型更新（改为资源收集页） |
| 2025年4月29日 | 🗑️ 删除根目录废弃文件 |
| 2025年4月29日 | ⏱️ 模拟时钟界面添加图标和全屏按钮 |
| 2025年4月29日 | 📊 资源收集界面添加页面图标 |
| 2025年4月29日 | 🐞 修复资源收集界面图标不显示BUG |
| 2025年4月29日 | ✨ 资源收集界面更换更好的图标 |
| 2025年4月29日 | 🖱️ 添加鼠标与页面的交互 |
| 2025年5月2-5日 | 📊 资源收集界面多项更新：添加更多资源、分类优化、美化界面、优化逻辑、添加时钟 |
| 2025年5月12日 | 📜 添加GPL-3.0许可证和运营日志 |
| 2025年5月13日 | 🔗 修饰所有跳转链接格式（改为相对路径） |
| 2025年6月13日 | 🎨 美化自述文件 |

## 🚀 快速开始

### 本地预览

```bash
# 克隆仓库
git clone https://github.com/Zmh20121211/21306.top.git
cd 21306.top

# 使用任意静态服务器预览
python -m http.server 8080
# 或
npx serve .
```

### 部署到 GitHub Pages

1. 在仓库 Settings → Pages 中启用 GitHub Pages
2. 选择部署分支（`main`）和根目录
3. 访问 `https://21306.top` 或 `https://Zmh20121211.github.io/21306.top`

## 🤝 贡献

欢迎提交 Issue 和 Pull Request！

1. Fork 本仓库
2. 创建功能分支（`feature/描述`）
3. 提交更改
4. 推送到分支
5. 提交 Pull Request

## 📄 许可证

本项目基于 **GPL-3.0** 许可证分发。详见 [LICENSE](LICENSE) 文件。

## ⭐ Star 历史

[![Star History Chart](https://api.star-history.com/svg?repos=Zmh20121211/21306.top&type=Date)](https://star-history.com/#Zmh20121211/21306.top&Date)

---

<div align="right">
  <strong>最后更新:</strong> 2025年6月13日
</div>
